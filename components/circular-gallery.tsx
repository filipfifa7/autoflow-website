...
class App {
  ...
  onActiveIndexChange?: (index: number, item: CircularGalleryItem) => void;
  ...
  onCheck() {
    ...
    if (this.baseItems.length && this.onActiveIndexChange) {
      const baseCount = this.baseItems.length;
      const normalizedIndex = ((itemIndex % baseCount) + baseCount) % baseCount;
      const activeItem = this.baseItems[normalizedIndex];
      this.onActiveIndexChange(normalizedIndex, activeItem);
    }
  }
...
type CircularGalleryProps = {
  ...
  scrollSpeed?: number;
  scrollEase?: number;
  onImageClick?: (index: number, item: CircularGalleryItem) => void;
};
  ...
  return (
    <div
      ...
      onClick={() => {
        ...
        const normalizedIndex = ((activeIndex % baseCount) + baseCount) % baseCount;
        const current = items[normalizedIndex];
        if (onImageClick) {
          onImageClick(normalizedIndex, current);
        }
      }}
    />
  );
